 ...
 * 
 * TWO EXECUTION PATHS:
 * 1. Server-side (/execute endpoint) - for unedited LLM-generated commands
 * 2. File-based (jarvis_executor.sh) - for user-edited commands
 * 
 * ARCHITECTURE:
 * - Chat: User → /chat_sync → LLM response
 * - Generate: User query → /generate_sync → Command
 * - Execute (unedited): Command → /execute → Server executes → Result
 * - Execute (edited): Command → file-based → jarvis_executor.sh → Result
 */
class TermuxLlamaClient(
    ...
    // File-based executor configuration (for edited commands)
    ...
    /**
     * Execute command via server /execute endpoint
     * Used for UNEDITED LLM-generated commands
     * 
     * Server executes the command in Termux and returns result
     */
    suspend fun executeCommandViaServer(command: String): CommandResult = withContext(Dispatchers.IO) {
        try {
            ...
            connection.setRequestProperty("Content-Type", "application/json")
            connection.connectTimeout = 10000
            connection.readTimeout = 70000 // 70 seconds (server has 60s timeout)
            connection.doOutput = true
    ...
    /**
     * Execute command via file-based executor (jarvis_executor.sh)
     * Used for USER-EDITED commands
     * 
     * WORKFLOW:
    ...
    suspend fun executeCommandViaFile(command: String): CommandResult = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(tag, "File-based execution: exitCode=$exitCode")
    ...
    /**
     * LEGACY: Execute command (kept for backward compatibility)
     * Defaults to file-based execution
     */
