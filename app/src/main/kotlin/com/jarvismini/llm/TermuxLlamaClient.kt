...
/**
 * FIXED: Improved file-based execution with better sync handling
 * 
 * Changes:
 * - Clear result file completely before execution
 * - Add initial delay for executor startup
 * - Better file sync handling
...
class TermuxLlamaClient(
    ...
    /**
     * FIXED: Improved file-based execution
     * - Better file sync handling
     * - Result validation
     * - Detailed logging
     */
    suspend fun executeCommandViaFile(command: String): CommandResult = withContext(Dispatchers.IO) {
        try {
            ...
            // Clear old result completely
            Log.d(tag, "Clearing old result...")
            if (resultFile.exists()) {
                resultFile.delete()
                delay(20) // Let file system catch up
            }
            resultFile.createNewFile()
            resultFile.writeText("")
            Log.d(tag, "Result file cleared")
            
            // Write command
            commandFile.writeText(command)
            Log.d(tag, "Command written, waiting for result...")
            
            // Small initial delay for executor to start processing
            delay(100)
            
            // Wait for result with improved checking
            var attempts = 0
            val maxAttempts = 300 // 60 seconds (300 * 200ms)
            ...
            while (attempts < maxAttempts) {
                try {
                    if (resultFile.exists()) {
                        ...
                        if (length > 0) {
                            resultContent = resultFile.readText().trim()
                            
                            // Valid result MUST contain pipe separator
                            if (resultContent.isNotEmpty() && resultContent.contains("|")) {
                                Log.d(tag, "✓ Valid result found after ${attempts * 200}ms")
                                Log.d(tag, "  Format: ${resultContent.take(50)}...")
                                break
                            } else if (resultContent.isNotEmpty()) {
                                Log.d(tag, "  Result file has content but no pipe: '$resultContent'")
                            }
                        }
                    }
                } catch (e: Exception) {
                    Log.d(tag, "  Read attempt failed (will retry): ${e.message}")
                }
                ...
                if (attempts % 25 == 0) {
                    Log.d(tag, "⏳ Still waiting... ${attempts * 200}ms elapsed")
                }
            }
            
            // Check timeout or invalid result
            if (resultContent.isNullOrEmpty() || !resultContent.contains("|")) {
                Log.e(tag, "❌ TIMEOUT or INVALID RESULT")
                Log.e(tag, "  Time elapsed: ${attempts * 200}ms")
                Log.e(tag, "  Result content: '$resultContent'")
                Log.e(tag, "  Check executor: tail -f /sdcard/jarvis/executor.log")
                
                return@withContext CommandResult(
                    success = false,
                    error = "Failed to get result.\n\nLast content: '$resultContent'\n\nCheck executor log:\n  tail -f /sdcard/jarvis/executor.log",
                    exitCode = -1,
                    method = "file-based"
                )
            }
            ...
            if (parts.size != 2) {
                Log.e(tag, "❌ Invalid result format: $resultContent")
                return@withContext CommandResult(
                    success = false,
            ...
            Log.e(tag, "❌ File-based execution exception", e)
            CommandResult(
                success = false,
