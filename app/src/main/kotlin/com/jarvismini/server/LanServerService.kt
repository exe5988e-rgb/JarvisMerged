...
class LanServerService : Service() {
    ...
    // Use port 8081 to avoid conflict with llama.cpp server on 8080
    ...
    override fun onCreate() {
        ...
        try {
            ...
            val notification = createNotification("Starting on port $serverPort...")
            ...
            try {
                ...
                Log.i(tag, "✅ LAN Server started on port $serverPort")
                
                val successNotification = createNotification("Running on port $serverPort")
                getSystemService(NotificationManager::class.java)?.notify(NOTIFICATION_ID, successNotification)
                
            } catch (e: Exception) {
                Log.e(tag, "❌ Failed to start LAN server", e)
                val errorNotification = createNotification("Failed: ${e.message}")
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Allows LAN devices to access JARVIS on port $serverPort"
                setShowBadge(false)
                enableLights(false)
